...
export default function StatsModal({
  ...
  const labels = distLabels ?? stats.distribution.map((_, i) => String(i + 1))

  return (
    <Modal title="סטטיסטיקה" onClose={onClose}>
      <div className="stat-row">
        <Stat value={stats.played} label="משחקים" />
        <Stat value={`${winRate}%`} label={rateLabel} />
        <Stat value={stats.currentStreak} label="רצף נוכחי" />
        <Stat value={stats.maxStreak} label="רצף שיא" />
      </div>

      <h3 className="dist-title">{distTitle}</h3>
      <ul className="dist">
        {stats.distribution.map((count, i) => (
          <li key={i}>
            <span className="dist-label">{labels[i]}</span>
